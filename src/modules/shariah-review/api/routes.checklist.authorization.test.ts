...
describe('PUT /api/v1/shariah-reviews/:reviewId/checklist', () => {
  ...
  // New test: reviewer-denied shared audit
  test('should persist shared access audit event for forbidden checklist save due to reviewer denial', async () => {
    const repository = new InMemoryShariahReviewRepository();
    const roleAssignmentRepository = new InMemoryRoleAssignmentRepository();
    const roleRepository = new InMemoryRoleRepository();
    ...
    const response = await server.inject({
      ...
      payload: checklistPayload,
      headers: {
        'x-actor-id': 'userWithNoAssignment' // Unauthorized user
      }
    });
    ...
    assert.strictEqual(event.actorUserId, 'userWithNoAssignment');
