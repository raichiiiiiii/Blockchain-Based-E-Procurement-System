...
type CompletenessStatus = 'complete' | 'partial' | 'unknown';

const SEQUENCE_FIELD_NAMES = [
  'eventId',
  'occurredAt',
  'actorUserId',
  'action',
...
const AccessEventSequencePage: React.FC = () => {
  ...
  const [completenessStatus, setCompletenessStatus] = useState<CompletenessStatus>('unknown');

  const activeDemoValue = mode === 'actor' ? actorUserId.trim() : targetType.trim();
  ...
  return (
    <div className="page-container">
      ...
      <div className="demo-controls">
        ...
        <p>Enter deterministic scaffold values in Actor User ID or Target Type to simulate states:</p>
        <ul>
          <li><strong>"complete"</strong> → Success with backend-provided complete metadata</li>
          <li><strong>"partial"</strong> → Partial completeness warning</li>
          <li><strong>"unknown"</strong> → Unknown completeness warning</li>
          <li><strong>"empty"</strong> → Empty result state</li>
      ...
      {sequenceState === 'incompleteOrUnknown' && (
        <div className="incomplete-section">
          ...
          <div className="completeness-warning">
            ...
            <p>Reason: completeness_not_proven</p>
            <p>Available events may be returned by the backend, but this dashboard scaffold does not prove the sequence is complete.</p>
          </div>
          <div className="schema-preview">
            <h4>Backend Sequence Schema Preview</h4>
            <ul>
              {SEQUENCE_FIELD_NAMES.map(fieldName => (
                <li key={fieldName}><code>{fieldName}</code> — backend-provided value only</li>
              ))}
            </ul>
          </div>
      ...
      {sequenceState === 'successWithItems' && (
        <div className="result-section">
          ...
          <div className="completeness-info">
            ...
            <p>
              Message: All events matching the scope and time window are included only when backend completeness metadata proves this state.
            </p>
          </div>
          <div className="schema-preview">
            <h4>Backend Sequence Schema Preview</h4>
      ...
      <div className="info-section">
        ...
        <ul>
          ...
          <li>Reason: completeness_not_proven when completeness is not proven</li>
