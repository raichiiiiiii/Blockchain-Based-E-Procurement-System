...
This map links the current MVP requirement families to backlog items, code areas, and evidence. It is intentionally compact; detailed acceptance criteria remain in `backlog/backlog.csv` and `backlog/production-extension-roadmap.csv`.
...
| R15 | Regulator export and evidence bundle | PBI-015, PBI-406 to PBI-412, PBI-448 | `src/modules/reporting/`, `docs/contracts/EXPORT_SIGNING_KEY_MANAGEMENT_CONTRACT.md` | export bundle route tests, PBI-448 evidence |
...
| R22 | Audit, access history, proof, non-repudiation | PBI-022, PBI-309 to PBI-340, PBI-421, PBI-437 | `src/modules/shared/`, `src/modules/blockchain/`, `chaincode/audit-anchor/` | chaincode tests, blockchain route tests, production Fabric evidence |
...
- PBI-438 remains open because live production Fabric consortium operation is not validated.
- PBI-452 and PBI-462 remain open in the production-extension roadmap.
