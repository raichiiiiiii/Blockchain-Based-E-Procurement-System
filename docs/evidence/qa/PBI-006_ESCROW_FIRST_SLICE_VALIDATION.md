...
The first targeted test run failed because the test compared the unprefixed lifecycle payload hash with normalized blockchain metadata. The assertion was corrected to compare against the proof-facing anchor hash, and the targeted suite then passed.

Full test result:

```text
npm test: 632 tests passed, 0 failed
```
