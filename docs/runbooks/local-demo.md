...
- KYC/AML cases, export bundles, Shariah reviews, PLS contracts, operational incidents, document metadata, contract negotiation records, external API credentials/idempotency/audit, and sandbox/manual payment instructions are now PostgreSQL-wired in runtime mode. Later persistence hardening remains for ERP/accounting jobs.
